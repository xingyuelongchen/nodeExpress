<template>
  <div class="vue-image-crop-upload" v-show="value">
    <div class="vicp-wrap">
      ...
      <!-- <my-upload field="logo" ref="profile"             // name
                  @crop-success="cropSuccess"                // 裁剪成功
                  @crop-upload-success="cropUploadSuccess"   // 上传成功
                  @crop-upload-fail="cropUploadFail"         // 上传失败
                  v-model="showEditLogo"                     // 控件是否显示
                  :width="200"                               // 裁剪图片的宽
                  :height="200"                              // 裁剪图片的高
                  :headers="headers"                         // 添加请求头
                  img-format="png">                          // 图片上传格式
      </my-upload> -->
        ...
        <div class="vicp-crop">
          <div class="vicp-crop-left" v-show="true">
            <div class="vicp-img-container">
              <img :src="sourceImgUrl" :style="sourceImgStyle" class="vicp-img" draggable="false" @drag="preventDefault"
                   @dragstart="preventDefault" @dragend="preventDefault" @dragleave="preventDefault"
                   @dragover="preventDefault" @dragenter="preventDefault" @drop="preventDefault"
                   @touchstart="imgStartMove" @touchmove="imgMove" @touchend="createImg" @touchcancel="createImg"
...
<script>
  ...
  export default {
    props: {
      ...
      ki: {default: 0},
      value: {default: true}, // 显示该控件与否
      url: {type: String, default: ''}, // 上传地址
      params: {type: Object, default: null}, // 其他要上传文件附带的数据，对象格式
      headers: {type: Object, default: null}, // Add custom headers
      width: {type: Number}, // 剪裁图片的宽
      height: {type: Number}, // 剪裁图片的高
      noRotate: {type: Boolean, default: true}, // 不显示旋转功能
      noCircle: {type: Boolean, default: true}, // 不预览圆形图片
      noSquare: {type: Boolean, default: false}, // 不预览方形图片
      maxSize: {type: Number, default: 10240}, // 单文件大小限制
      langType: {type: String, default: 'zh'}, // 语言类型
      langExt: {type: Object, default: null}, // 语言包
      imgFormat: {type: String, default: 'png'}, // 图片上传格式
      withCredentials: {type: Boolean, default: false}, // 是否支持跨域
      multiple: false,
    },
    data() {
      let that = this,
      ...
      return {
        mime, // 图片的mime
        lang, // 语言包
        isSupported, // 浏览器是否支持该控件
        isSupportTouch: document.hasOwnProperty('ontouchstart'), // 浏览器是否支持触屏事件
    ...
    computed: {
      ...
      ...mapState(['imagesToUpload', 'filesToUpload']),
     ...
     ---------------------------------------------------------------*/
      ...
      setSourceImg(file) {
        ...
        fr.onload = e => {
          ...
          if (this.width === undefined) {
            that.createImgUrl = fr.result
            let index = this.filesToUpload.findIndex(value => {
              return value.field === field
            })

            if (index === -1) {
              store.commit('setFilesToUpload', {
                field: this.field,
                createImgUrl: that.createImgUrl
              })
            } else {
              store.commit('setSpliceFilesToUpload', index)
              store.commit('setFilesToUpload', {
                field: this.field,
                createImgUrl: that.createImgUrl
              })
            ...
            that.sourceImgUrl = fr.result // 拿到图片源
            that.startCrop() // 裁剪前准备工作
          }
        }
        fr.readAsDataURL(file)
      },
      // // 设置图片源用于裁剪
      // setSourceImg111(file) {
      ...
      startCrop() {
        ...
        img.onload = function () {
          ...
          that.sourceImg = img
          that.createImg() // 生成需求图片
          that.setStep(2) // 生成图片后就进入第二界面
        }
      },
      // 鼠标按下图片准备移动
      imgStartMove(e) {
      ...
      createImg(e) {
        ...
        let quality = 30
        that.createImgUrl = canvas.toDataURL(mime, 0.5) // 创建好的图片
        // -------- 自己加入的   加入自己的store图片数组里
        let index = this.filesToUpload.findIndex(value => {
          return value.field === field
        })
        if (index === -1) {
      ...
      // 点击保存上传开始
      prepareUpload() {
        let {url, createImgUrl, field, ki, mime} = this
        this.$emit('crop-success', createImgUrl, field, ki) // 触发裁剪成功事件
        let index = this.imagesToUpload.findIndex(value => {
          return value.key === field
        })
        if (this.multiple) {
          store.commit('setImagesToUpload', {
            key: field,
            value: data2blob(this.createImgUrl, mime),
            createImgUrl: this.createImgUrl
          })
          this.$emit('imagesToUpload', {
            key: field,
            value: data2blob(this.createImgUrl, mime),
            createImgUrl: this.createImgUrl
          });
          ...
          if (index === -1) {
            ...
            store.commit('setSpliceToUpload', index)
            store.commit('setImagesToUpload', {
              key: field,
              value: data2blob(this.createImgUrl, mime)
            })
        ...
        // 如果没有url或者myUrl 就取消上传
        if (
          (typeof url == 'string' && url) ||
          (typeof this.myUrl == 'string' && this.myUrl)
        ) {
          this.upload()
        } else {
          this.off()
      ...
      upload() {
        ...
        // -------- 自己加入的    将store的图片数组加入fmData中
        store.state.filesToUpload.forEach((currentValue, i) => {
          if (currentValue.field === 'file') {
            fmData.append(
              currentValue.field,
              data2blob(currentValue.createImgUrl, mime),
              currentValue.field + '.' + imgFormat
            )
          } else {
            fmData.append(
              currentValue.field,
        ...
        // -------- 自己加入的    添加其他参数
        if (typeof this.myParams == 'object' && this.myParams) {
          Object.keys(this.myParams).forEach(k => {
            fmData.append(k, this.myParams[k])
          })
        }
        ...
        new Promise((resolve, reject) => {
          ...
          if (url === '') {
            client.open(this.myType, this.myUrl, true)
          } else {
            client.open(this.myType, url, true)
          }
          ...
          client.upload.addEventListener(
            'progress',
            uploadProgress,
            false
          ) //监听进度
...
<style>
  ...
  .vicp-img {
    ...
    -moz-customer-select: none;
    -ms-customer-select: none;
  ...
  .vue-image-crop-upload .vicp-wrap .vicp-operate a {
    ...
    font-size: 14px;
    color: #3c3c3c;
    background-image: linear-gradient(45deg, #CFD8DC 0%, #2c7a90 100%),
    linear-gradient(#CFD8DC, #2c7a90) !important;
