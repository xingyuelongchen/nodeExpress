...
function post(req, res, config) {
    ...
    res.Api.db.find(find, (err, d, count) => {
        if (d.length > 0 && d[0].password == res.MD5(password)) {
            let token = res.MD5(res.MD5(d[0].password) + res.MD5(d[0]._id));
            // 不返回敏感信息
            delete d[0].password;
            let user = d[0];
            ...
            // 查询当前账户是否已有token信息
            res.Api.db.find({ table: config.db.table.uid_id, token }, (err, e) => {
                if (err) {
                    res.info('登陆失败')
                    return
                }
                // 未登录，添加当前token
                if (e.length == 0) {
                    res.Api.db.insert(config.db.table.uid_id, data, (err, data) => {
                        res.succress({ message: '登陆成功', content: user })
                    })
                }
                // 已登录，更新token
                if (e.length > 0) {
                    ...
                    res.Api.db.update(option, (err, data) => {
                        if (err) {
                            res.error(500);
                        } else {
                            res.succress({ message: '登陆成功', content: user })
